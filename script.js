const localStorageKey = 'to-do-list-wc'

function validacao(){
    let values = JSON.parse(localStorage.getItem(localStorageKey) || "[]")
    let inputValue = document.getElementById('input_nova_tarefa').value
    let exist = values.find(x => x.name == inputValue)
    return !exist ? false : true
}

function novaTarefa(){
    let input = document.getElementById('input_nova_tarefa')

    if(!input.value){
        alert('É necessário digitar uma tarefa para ser inserida em sua lista!')
    }
    else if(validacao()){
